...
func (s *Service) SwapDatapoolHolders(datapoolid string, data int32, oldHolders []string) error {
	...
	for _, holderid := range oldHolders {
		s.DeleteDatapoolAndHolderOnLocal(datapoolid, holderid)
		if !s.WithoutConnectRemoteForTest {
			s.DeleteDatapoolOnRemote(datapoolid, holderid)
		}
	}
	if len(oldHolders) > 0 {
...
//ローカルのデータプールを削除
func (s *Service) DeleteDatapoolAndHolderOnLocal(datapoolid string, workerId string) error {
...
func (s *Service) AddHolders(datapoolid string, data int32, num int) ([]*accounting.Worker, error) {
	...
	for _, worker := range newholders {
		if !s.WithoutConnectRemoteForTest {
			...
			go func(target *accounting.Worker) {
				...
				if err != nil {
					log2.Err.Printf("failed to send CreateDatapool request to %s\n", target.Addr)
					return
				}
	...
	if len(holders)-alreadyHolds < num {
		log2.Err.Printf("send request is %d but response is %d\n", num, len(holders)-alreadyHolds)
