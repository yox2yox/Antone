...
func New(config *config.BridgeConfig, debug bool, faultyFraction float64, credibilityThreshold float64, reputationResetRate float64, setWathcer bool, blackListing bool, stepVoting bool, skipValidation bool, initialReputation int, workerAttackMode int, sabotagableReputation int, exportReputations int) (*Peer, error) {
	...
	{ //setup orders
		peer.Orders = orders.NewService(peer.Accounting, peer.Datapool, debug, peer.Config.Order.CalcErrorRate, setWathcer, stepVoting, skipValidation, workerAttackMode, sabotagableReputation, exportReputations)
